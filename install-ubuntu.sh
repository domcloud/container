...
# Installations
apt-get update
apt-get -y install bzip2 bison btop chromium clang certbot cmake fd-find git ncdu htop iftop ipset jq lsof make nano ninja-build ncurses-bin nodejs patch ripgrep ruby rsync screen socat strace tar time tmux vim wget whois xz-utils zstd \
...
if [[ -n "$OPTIONAL_INSTALL" ]]; then
  ...
  echo "deb [signed-by=$GPGDIR/microsoft.gpg] https://packages.microsoft.com/ubuntu/$CODEVER/prod $CODENAME main" | tee /etc/apt/sources.list.d/microsoft.list
