...
apt-get -y install webmin-{virtual-server,virtualmin-nginx,virtualmin-nginx-ssl,ruby-gems} valkey-server earlyoom fail2ban mariadb-server nftables bind9 sudo openssh-server systemd-container
